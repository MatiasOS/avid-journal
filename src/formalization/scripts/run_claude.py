...
# AViD: force UTF-8 with errors="backslashreplace" so prints of Lean output
# (containing exists/forall/leq/Spanish accents) do not crash on Windows
# consoles using cp1252 / cp850.
try:
...
class ClaudeRunner:
    ...
    def run(
        ...
        # AViD: distinguish Anthropic rate-limit from generic failure so the
        # orchestrator can abort the entire paper run instead of marking
        # individual blocks as failed when Claude is unavailable. Exit 99
        # is reserved for "external service unavailable, retry later".
